...
async function ensureImageCapabilities(settings, apiKeyRef) {
  const section = await settings.get(SETTINGS_NS);
  if (section?.providers?.[PROVIDER_ID]?.apiKeyEnv !== apiKeyRef) return false;

  const mutations = imageInputOverrideMutations(section);
  if (mutations.length === 0) return false;

  await settings.mutate(SETTINGS_NS, mutations);
  return true;
}
